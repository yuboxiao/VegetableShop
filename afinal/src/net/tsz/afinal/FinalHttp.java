...
import net.tsz.afinal.http.HttpHandlerTask;
...
public class FinalHttp {
    ...
    */
     ...
     * @param callBack 回调方法
     */
    public void post(String url, Object object, AjaxCallBack<? extends Object> callBack) {
        post(url, objectToEntity(object), null, callBack);
    }
  ...
  //------------------put 请求-----------------------
    ...
    //---------------------下载---------------------------------------
    public HttpHandlerTask<File> download(String url,String target,AjaxCallBack<File> callback){
    	return download(url, null, target, false, callback);
    }
    

    public HttpHandlerTask<File> download(String url,String target,boolean isResume,AjaxCallBack<File> callback){
    	 return download(url, null, target, isResume, callback);
    }
    
    public HttpHandlerTask<File> download( String url,AjaxParams params, String target, AjaxCallBack<File> callback) {
   	 	return download(url, params, target, false, callback);
    }
    
    public HttpHandlerTask<File> download(String url,AjaxParams params, String target,boolean isResume, AjaxCallBack<File> callback) {
    	final HttpGet get =  new HttpGet(getUrlWithQueryString(url, params));
    	HttpHandlerTask<File> handler = new HttpHandlerTask<File>(httpClient, httpContext, callback,charset);
    ...
    protected <T> void sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, AjaxCallBack<T> ajaxCallBack) {
        ...
        HttpHandlerTask<T> httpHandler = new HttpHandlerTask<T>(client, httpContext, ajaxCallBack,charset);
        httpHandler.executeOnExecutor(executor, uriRequest);
    }
    ...
    private HttpEntity objectToEntity(Object object) {
        HttpEntity entity = null;

        if(object != null) {
            try {
				entity = new StringEntity(new Gson().toJson(object),"utf-8");
